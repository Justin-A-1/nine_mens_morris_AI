...
def MiniMaxOpeningBlack(board: str, depth: int, is_maximizing: bool):
    if depth == 0:
        return StaticEstOpeningBlack(board), board, 1
    ...
    if is_maximizing:
        ...
        for child in GenerateMovesOpeningBlack(board):
            eval, _, child_positions_evaluated = MiniMaxOpeningBlack(child, depth - 1, False)
        ...
        for child in GenerateMovesOpening(board):
            eval, _, child_positions_evaluated = MiniMaxOpeningBlack(child, depth - 1, True)
            positions_evaluated += child_positions_evaluated
            if eval < min_eval:
                min_eval = eval
                best_board = child
...
def main(input_file: str, output_file: str, depth: int):
    ...
    eval, best_board, positions_evaluated = MiniMaxOpeningBlack(board, depth, True)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 4:
        print("Usage: python MiniMaxOpeningBlack.py <input_file> <output_file> <depth>")
    else:
